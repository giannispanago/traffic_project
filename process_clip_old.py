...
for result in results:
    ...
    if boxes is not None and boxes.id is not None:
        ...
        for box, track_id in zip(boxes, track_ids):
            ...
            # μικρό display id μόνο για την οθόνη
            ...
            # σημείο μέτρησης: bottom-center
            cx = int((x1 + x2) / 2)
            cy = int(y2-5)

            # φτιάξε record για νέο όχημα
            ...
            if (
                ...
                and vehicles[track_id]["speed_kmh"] is None
            ):
                ...
                if time_sec > 0:
                    ...
                    # direction / ρεύμα
                     ...
                     # χρόνος του crossing σε sec από την αρχή του clip
                     crossing_time_sec = crossing_frame / FPS

                     # 5-minute window (1, 2, 3, ...)
                     window_5min = int(crossing_time_sec // 300) + 1
                     ...
                     vehicles[track_id]["time_sec"] = crossing_time_sec
                     vehicles[track_id]["window_5min"] = window_5min

                     print(
                     f"Vehicle {display_id} speed = {speed_kmh:.2f} km/h | "
                     f"{direction} | window {window_5min}"
                 )

            speed_text = ""
            if vehicles[track_id]["speed_kmh"] is not None:
                speed_text = f" | {vehicles[track_id]['speed_kmh']:.1f} km/h"
...
with open("vehicle_results.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f, delimiter=";")
    writer.writerow(["track_id", "class_name",  "line1_frame", "line2_frame", "crossing_frame" , "time_sec" , "window_5min" , "direction" , "speed_kmh"])
...
print("Saved results to vehicle_results.csv")